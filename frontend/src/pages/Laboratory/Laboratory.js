...
function Laboratory() {
    ...
    const getAppointmentList = async () => {
      ...
      if (subadmin_key) {
        ...
        const res = await fetch(
          ...
          `http://103.165.118.71:8085/getAllLaboratories`
        );
  ...
  const handleDownloadExcel = () => {
    window.open("http://103.165.118.71:8085/downloadLaboratories", "_blank");
  };
  ...
  const confirmDelete = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setDeleteId(null);
        console.log("Laboratory soft deleted successfully");
        window.location.reload();
      } else {
        console.error("Failed to soft delete laboratory");
      }
    } catch (error) {
      console.error("Error soft deleting laboratory:", error);
    }
  };
