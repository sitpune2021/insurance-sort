...
function Appointment() {
  ...
  useEffect(() => {
    const getAppointmentList = async () => {
      ...
      if (!lab_id) {
        console.error("Lab ID not found in session storage");
        return;
      }

      try {
        const res = await fetch(
          `http://103.165.118.71:8085/getallappointmentforAssistant?lab_id=${lab_id}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
        ...
        console.log("Fetched Appointments:", getData); // ✅ Log API response
