...
function Appointment() {
  ...
  const [subadminId, setSubadminId] = useState(id);
  ...
  const handleAssignTechnician = async () => {
    ...
    try {
      const response = await fetch("http://103.165.118.71:8085/assignToSubAdmin", {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    const getAppointmentList = async () => {
      ...
      if (tokenKey) {
        try {
          // Make API call with token_key
          const res = await fetch(
            `http://103.165.118.71:8085/getAllAppointmentsSubAdmin?token_key=${tokenKey}`
          );
        ...
        // Handle case where token_key is not found in sessionStorage
        console.error("No token_key found in sessionStorage");
      }
    };
